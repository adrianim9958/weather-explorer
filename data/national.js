window.NATIONAL_DATA = [
    {
        name: "가리산",
        address: "강원도 홍천군 두촌면ㆍ화촌면, 춘천시 북산면ㆍ동면",
        height: "1050",
    },
    {
        name: "가리왕산",
        address: "강원도 정선군 북평면ㆍ정선읍 회동리, 평창군 진부면",
        height: "1561",
    },
    {
        name: "가야산",
        address: "경상남도 합천군ㆍ거창군, 경상북도 성주군",
        height: "1432",
    },
    {
        name: "가지산 ",
        address: "울산광역시 울주군, 경상북도 청도군, 경상남도 밀양시",
        height: "1240",
    },
    {
        name: "감악산",
        address: "경기도 파주시 적성면, 양주시 남면, 연천군 전곡읍",
        height: "674",
    },
    {
        name: "강천산 ",
        address: "전라북도 순창군 팔덕면, 전라남도 담양군 용면",
        height: "585",
    },
    {
        name: "계룡산",
        address: "대전광역시, 충청남도 공주시 계룡면, 논산시 상월면, 계룡시 신도안면",
        height: "846",
    },
    {
        name: "계방산",
        address: "강원도 홍천군 내면, 평창군 용편면ㆍ진부면",
        height: "1579",
    },
    {name: "공작산", address: "강원도 홍천군 동면, 화촌면", height: "887"},
    {
        name: "관악산",
        address: "서울특별시 관악구, 경기도 안양시, 과천시",
        height: "632",
    },
    {
        name: "구병산",
        address: "경상북도 상주신 화북면, 충청북도 보은군 마로면ㆍ속리산면",
        height: "876",
    },
    {name: "금산", address: "경상남도 남해군 상주면, 이동면, 삼동면", height: "704"},
    {
        name: "금수산",
        address: "충청북도 제천시 수산면, 단양군 적성면",
        height: "1015",
    },
    {
        name: "금오산",
        address: "경상북도 구미시, 칠곡군 북삼읍, 김천시 남면",
        height: "976",
    },
    {
        name: "금정산 ",
        address: "부산광역시 금정구ㆍ북구, 경상남도 양산시",
        height: "800",
    },
    {name: "깃대봉", address: "전라남도 신안군 흑산면 홍도", height: "367"},
    {name: "남산(금오산)", address: "경상북도 경주시 남산동, 내남면", height: "495"},
    {
        name: "내연산 ",
        address: "경상북도 포항시 송라면ㆍ청하면ㆍ죽장면, 영덕군 남정면",
        height: "711",
    },
    {
        name: "내장산(신선봉)",
        address: "전라북도 정읍시 내장동, 순창군 쌍치면ㆍ복흥면",
        height: "763",
    },
    {
        name: "대둔산",
        address: "충청남도 논산시 벌곡면ㆍ금산군 진산면, 전라북도 완주군 운주면",
        height: "879",
    },
    {name: "대암산", address: "강원 양구군 동면, 인제군 서화면", height: "1312"},
    {
        name: "대야산",
        address: "경상북도 문경시 가은읍, 충청북도 괴산군 청천면",
        height: "931",
    },
    {name: "덕숭산(수덕산)", address: "충청남도 예산군 덕산면", height: "495"},
    {
        name: "덕유산(향적봉)",
        address: "전라북도 무주군ㆍ장수군, 경상남도 거창군ㆍ함양군",
        height: "1614",
    },
    {
        name: "덕항산",
        address: "강원도 삼척시 신기면, 태백시 하사미동",
        height: "1072",
    },
    {name: "도락산", address: "충청북도 단양군  단성면, 대강면", height: "965"},
    {
        name: "도봉산(자운봉)",
        address: "서울특별시 도봉구, 경기도 의정부시 호원동ㆍ양주시 장흥면",
        height: "740",
    },
    {
        name: "두륜산",
        address: "전라남도 해남군 삼산면ㆍ북일면ㆍ북평면ㆍ현산면",
        height: "700",
    },
    {
        name: "두타산",
        address: "강원도 동해시 삼화동, 삼척시 미로면ㆍ하자면",
        height: "1357",
    },
    {name: "마니산", address: "인천광역시 강화군 화도면", height: "472"},
    {
        name: "마이산(암마이산)",
        address: "전라북도 진안군 진안읍ㆍ마령면",
        height: "687",
    },
    {
        name: "명성산",
        address: "강원도 철원군 갈말읍, 경기도 포천시 영북면ㆍ이동면",
        height: "922",
    },
    {name: "명지산", address: "경기도 가평군 북면ㆍ하면", height: "1252"},
    {
        name: "모악산",
        address: "전라북도 김제시 금산면, 전주시 완산구, 완주군 구이면",
        height: "795",
    },
    {
        name: "무등산",
        address: "광주광역시 동구, 전라남도 담양군 남면ㆍ화순군 이서면",
        height: "1186",
    },
    {
        name: "무학산",
        address: "경상남도 창원시 교방동ㆍ두척동ㆍ내서읍",
        height: "761",
    },
    {name: "미륵산 ", address: "경상남도 통영시 산양읍ㆍ봉평동", height: "458"},
    {
        name: "민주지산",
        address: "충청북도 영동군 쌍촌면ㆍ용화면, 전라북도 무주군 설천면, 경상북도 김천시 부항면",
        height: "1241",
    },
    {
        name: "방장산",
        address: "전라북도 고창군 신림면ㆍ정읍시 입암면, 전라남도 장성군 ",
        height: "733",
    },
    {
        name: "방태산(주억봉)",
        address: "강원도 인제군 기린면ㆍ상남면, 홍천군 내면",
        height: "1445",
    },
    {
        name: "백덕산",
        address: "강원도 평창군 방림면, 횡성군 안흥면, 영월군 수주면",
        height: "1350",
    },
    {
        name: "백암산",
        address: "전라북도 순창군 복흥면, 잔라남도 장성군 북하면",
        height: "741",
    },
    {name: "백운산(포천)", address: "경기도 포천시 이동면", height: "903"},
    {
        name: "백운산(광양)",
        address: "전라남도 광양시 진상면ㆍ옥룡면ㆍ봉강면ㆍ다압면, 구례군 간전면",
        height: "1222",
    },
    {
        name: "백운산(정선)",
        address: "강원도 정선군 신동읍, 평창군 미탄면",
        height: "883",
    },
    {
        name: "변산(의상봉)",
        address: "전라북도 부안군 변산면, 상서면, 진서면",
        height: "459",
    },
    {
        name: "북한산(백운대)",
        address: "서울특별시 강북구ㆍ성북구ㆍ종로구ㆍ은평구, 경기도 고양시ㆍ양주시",
        height: "835",
    },
    {
        name: "비슬산(천왕봉)",
        address: "대구광역시 달성군 옥포면ㆍ유가면ㆍ가창면, 경상북도 청도군 각북면",
        height: "1083",
    },
    {name: "삼악산", address: "강원도 춘천시 서면", height: "655"},
    {
        name: "서대산",
        address: "충청남도 금산군 추부면ㆍ군북면, 충청북도 옥천군 군서면",
        height: "905",
    },
    {
        name: "선운산",
        address: "전라북도 고창군 아산면ㆍ심원면ㆍ해리면",
        height: "334",
    },
    {
        name: "설악산(대청봉)",
        address: "강원도 속초시 설악동, 인제군 북면ㆍ인제읍, 양양군 서면ㆍ강현면",
        height: "1708",
    },
    {name: "성인봉", address: "경상북도 울릉군 울릉읍 서면ㆍ북면", height: "986"},
    {
        name: "소백산",
        address: "경상북도 영주시 풍기읍, 충청북도 단양군 단양읍",
        height: "1439",
    },
    {name: "소요산", address: "경기도 동두천시, 포천시 신북면", height: "587"},
    {
        name: "속리산",
        address: "경상북도 상주시 화북면, 충청북도 보은군 내속리면",
        height: "1058",
    },
    {name: "신불산", address: "울산광역시 울주군 삼남면ㆍ상북면", height: "1159"},
    {name: "연화산", address: "경상남도 고성군 개천면ㆍ영현면", height: "524"},
    {
        name: "오대산(비로봉)",
        address: "강원도 평창군 진부면, 홍천군 내면, 강릉시 연곡면",
        height: "1565",
    },
    {name: "오봉산", address: "강원도 춘천시 북산면, 화천군 간동면", height: "777"},
    {name: "용문산", address: "경기도 양평군 용문면ㆍ옥천면", height: "1157"},
    {
        name: "용화산",
        address: "강원도 화천군 간동면ㆍ하남면, 춘천시 사북면",
        height: "877",
    },
    {
        name: "운문산",
        address: "경상북도 청도군 운문면, 경상남도 밀양시 산내면",
        height: "1195",
    },
    {
        name: "운악산(현등산)",
        address: "경기도 가평군 하면, 포천시 화현면",
        height: "934",
    },
    {
        name: "운장산",
        address: "전라북도 진안군 주천면, 부귀면, 정천면, 완주군 동상면",
        height: "1125",
    },
    {name: "월악산", address: "충청북도 제천시 한수면, 덕산면", height: "1095"},
    {
        name: "월출산",
        address: "전라남도 영암군 영암읍ㆍ군서면ㆍ학산면, 강진군 성전면",
        height: "810",
    },
    {name: "유명산", address: "경기도 가평군 설악면, 양평군 옥천면", height: "864"},
    {
        name: "응봉산(매봉산)",
        address: "강원도 삼척시 가곡면ㆍ원덕읍, 경상북도 울진군 북면",
        height: "999",
    },
    {name: "장안산", address: "전라북도 장수군 장수읍, 계남면", height: "1237"},
    {
        name: "재약산",
        address: "경상남도 밀양시 단장면ㆍ산내면, 울산광역시 울주군 상북면",
        height: "1119",
    },
    {name: "적상산", address: "전라북도 무주군 적상면", height: "1030"},
    {
        name: "점봉산",
        address: "강원도 양양군 서면, 인제군 인제읍ㆍ기린면",
        height: "1426",
    },
    {name: "조계산", address: "전라남도 순천시 승주읍ㆍ송광면", height: "887"},
    {
        name: "주왕산",
        address: "경상북도 청송군 청송읍ㆍ부동면, 영덕군 지품면ㆍ달산면",
        height: "722",
    },
    {name: "주흘산", address: "경상북도 문경시 문경읍", height: "1108"},
    {
        name: "지리산(천왕봉)",
        address: "경상남도 하동군ㆍ산청군ㆍ함양군, 전라북도 남원시, 전라남도 구례군",
        height: "1915",
    },
    {name: "지리산", address: "경상남도 통영시 사량면", height: "399"},
    {name: "천관산", address: "전라남도 장흥군 관산읍, 대덕읍", height: "724"},
    {name: "천마산", address: "경기도 남양주시 화도읍, 오남읍", height: "810"},
    {
        name: "천성산",
        address: "경상남도 양산시 하북면, 웅상읍, 상북면",
        height: "920",
    },
    {
        name: "천태산",
        address: "충청북도 영동군 양산면, 충청남도 금산군 제원면",
        height: "714",
    },
    {
        name: "청량산",
        address: "경상북도 봉화군 명호면ㆍ재산면, 안동시 도산면ㆍ예안면",
        height: "869",
    },
    {
        name: "추월산",
        address: "전라남도 담양군 용면, 전라북도 순창군 복흥면",
        height: "731",
    },
    {name: "축령산", address: "경기도 남양주시 수동면, 가평군 상면", height: "887"},
    {name: "치악산", address: "강원도 원주시, 횡성군, 영월군", height: "1282"},
    {
        name: "칠갑산",
        address: "충청남도 청양군 대치면, 정산면, 장평면",
        height: "559",
    },
    {
        name: "태백산",
        address: "강원도 태백시, 경상북도 봉화군 석포면",
        height: "1566",
    },
    {
        name: "태화산",
        address: "강원도 영월군 남면, 충청북도 단양군 영춘면",
        height: "1027",
    },
    {
        name: "팔공산",
        address: "경상북도 군위군 부계면, 영천시 신녕면, 대구광역시 동구",
        height: "1192",
    },
    {name: "팔봉산", address: "강원도 홍천군 서면", height: "328"},
    {name: "팔영산", address: "전라남도 고흥군 점암면, 영남면", height: "606"},
    {name: "한라산", address: "제주특별자치도", height: "1947"},
    {
        name: "화악산",
        address: "경기도 가평군 북면, 강원도 화천군 사내면",
        height: "1468",
    },
    {name: "화왕산", address: "경상남도 창녕군 창녕읍, 고암면", height: "757"},
    {
        name: "황매산",
        address: "경상남도 합천군 대병면ㆍ가회면, 산청군 차황면",
        height: "1113",
    },
    {
        name: "황석산",
        address: "경상남도 함양군 안의면, 서하면, 서상면",
        height: "1192",
    },
    {name: "황악산", address: "경상북도 김천시 대항면", height: "1111"},
    {name: "황장산", address: "경상북도 문경시 동로면", height: "1078"},
    {
        name: "희양산",
        address: "경상북도 문경시 가은읍, 충청북도 괴산군 연풍면",
        height: "996",
    },
];
